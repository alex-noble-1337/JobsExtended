...
Navigator
	...
	2nd tree
		Driving the sub and small range aoe buffs
	3rd tree
...
Tear gas
Passenger BaroPlus integration (alien things)
Mech gets robots
Eng + Mech gets Exoskeleton
Pas gets every small monster tamable
...
Movemnt speed buff when holding a sabre

Split more boring skills to an separwte skill tree, that toy pay with skill points (weapons, eng etc)
...
Cephalopod 🇺🇦 Oracle#4892 532596743976779783: desire confetti ammo which makes a nice noise instead of a gun sound when you fire with it so like, revolver loaded with clown bullets, makes a nice party blower noise when it fires, or maybe a party popper noise
...
CrazyIvan#9415 337307802794065932: I like this idea: generic dive suit underneath more situation-specific power armor. That way players who run out of power (immobilized) can discard the suit and return to the sub. Hard choices are good.
...
CrazyIvan#9415 337307802794065932: What if what if and hear me out The engine doesn't use power but instead its max speed is based on the reactor's current temperature. That'd give you an incentive to redline the reactor when you want to go fast.
...
]Victor2-2: Talents should be nerfed in raw power and instead to get even near the bonus we have now you should buy the same talent (or a better version from the talent tree) Like 4 times
...
Xo, check if cap is alive, rather than to check if he is nea, when kill something gain helm, when steering gain weapons
...
Eng. More depleted uranium munitions

Joint effort on getting blueprints, either for money or for materials + skill (hacking)

Cob fabrcator specifocly for electric components, + 1 when you fabricate one max 5 times when you complete a mission, better tools (electric screwdriver wrench, multitool), final tiers upgrade suits 
...
Make guitar holdable in invenory same as acodeon. For engi.
...
Jobs extended addons with mod that allowed reloading packages
